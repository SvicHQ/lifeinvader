...
document.addEventListener('DOMContentLoaded', () => {
    ...
    houseSubmitBtn.addEventListener('click', function () {
        ...
        if (mainFeatures.length > 0 || view) {
            adText += " with ";
            let featuresAndView = [...mainFeatures];
            if (view) featuresAndView.push(view);
            if (featuresAndView.length === 1) {
                adText += featuresAndView[0];
            } else {
                const allButLast = featuresAndView.slice(0, -1).join(", ");
                const lastFeature = featuresAndView[featuresAndView.length - 1];
        ...
        // Remove trailing period if present (to avoid 5 w.h..)
        adText = adText.replace(/\.\s*$/, "");

        // Now add the price/budget
    ...
    // Reset house ad form
    const houseResetBtn = document.getElementById('house-reset-btn');
    houseResetBtn.addEventListener('click', function() {
        // Reset all fields
